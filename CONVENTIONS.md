...
本文将提交与版本控制纪律（§1–§2）、SNL 身份与命名及库结构（§3–§11）、
书写语法与标签（§12–§13）、SNL-Lean 同步（§14）、本仓内容纪律（§15）
与 Agentic 工作流（§16）分别规定；另附调研参考。Agent 协作、任务范围及交接
不改变前述语言语义与归属规则。
...
16. [Agentic 工作流规范](#16-agentic-工作流规范)
...
（见 §2），完成检查后再合入。Agent 执行时的授权范围与交还边界见 §16。
...
- 任务分支放在独立 worktree 或独立 clone，不把阅读器 `--root` 当作临时 checkout
  （见 §1.4）。同一分支名在任意时刻至多被一个 worktree 占用；已有占用时不得强行
  checkout，应等其清理后重开，或使用另一分支名。
- 多 Agent 的 worktree 归属、分配范围与并发边界另见 §16.1；本节只规定版本控制隔离。

### 2.3 分支与工作区占用探测

开工前用 `git worktree list`、`git branch --show-current` 和 `git status --porcelain`
确认 worktree、分支及未清理改动；目标分支已被别的 worktree checkout 时按 §2.2
处理，不强行复用。`main` 在工作期间被推进时先对齐再继续。多 Agent 的占用归属不
因探测而自动转移，见 §16.1。
...
带下标的变量写成公式节点 `$p_0$`、`$p_1$`、`$q_0$`、`$f_0$`，不写裸 `p0`
或 `\mathsf{p0}`。声明用 `@$p_0$`，同条目引用用 `$p_0$`，保留独立 binder
及其局部绑定；必要时采用经实际解析器核验的局部树路径来源。不为变量排版新增领域宏，
也不把含变量的整段运算封成无法解析绑定的 opaque 公式。

跨 Entry 引用继续遵守 §12.4.1 的临时规则：Context 已声明的变量不重复声明、不挂
`@EntryId`，使用同样的公式名字。此临时无来源引用不等于已实现跨条目语义绑定。
复核时同时检查 AST 的 binder / bvar 身份及实际 LaTeX；仅显示相同不是绑定证据。
...
语境中同一作用域、同一完整类型的多个对象，必须用 `__list__[formula]` 合并到
一个 `Type.annotation`，每个对象仍保留自己的真实 binder。例如：

```snl
Type.annotation(__list__[formula](@a,@b,@c),X)
Type.annotation(__list__[formula](@$p_0$,@$p_1$),Path(a,b))
```

不得因显示简短而合并不同类型或不同索引的对象。例如 `Path(a,b)` 与 `Path(b,c)`
是不同的端点索引类型，必须分组声明；拓扑空间类型相同的载体可以合并声明，但各自的
`TopologicalSpace` 假设必须保留。仅合并同一可见作用域内的声明，不跨量词、局部假设
或 Entry 边界强行合并。变更结构后须重新核对局部 binder 的坐标与引用。

#### 12.4.3 定义谓词用 `def[prop]`
...
### 15.4 自然语言补充必须独立为所属 Entry 的 remark 子条目

- 动机、背景、导读、几何直观、历史说明、记号及拓扑约定等**自然语言补充内容必须**
  独立为 `remark`，不塞进所属 Entry 的正文（无论 `snl` 或 `markdown`），也不塞进章节壳。
  remark 与所属 Entry 同 Package，按 §11.4 作为其 `branch` 子条目，用 `Subentry`
  计数器挂载，并紧跟所属 Entry，先于其其他子条目及后续定义。
- 不得丢失必要假设、定义数据或数学内容。定义性措辞及必要假设本身不是“补充”，仍须
  在正式声明或可见语境中完整保留。标准拓扑、子空间拓扑等约定可以集中在所属概念的
  remark 中，但依赖它们的语境须明确沿用，不得退成任意拓扑。
- 连续性、等价关系或良定义性的论证不是可删动机。补充论证移到 remark 时必须完整
  保留并明确标为论证；独立正式证明可用 proof Entry。删除动机不得使数学内容或证明
  出现缺口，也不得把“移出正文”当成“删除证明”。
- 始终是散文的补充放在 remark 的 `content.markdown`，不硬塞进 `snl`。
- **语义归属优先于四层树约定。** 围绕一个定义的性质应挂为该定义的子条目；性质的
  证明挂在该性质下，注记挂在其直接说明的条目下。不得仅为压缩层数，将这些性质
  提升为定义的同级条目，或将证明、注记移离其所属条目。
- Section → Subsection → Entry → Subentry 是常见布局，不是硬性深度上限。
  因性质、性质的证明或注记形成第五层及更深层级是允许的；例如
  Section → Subsection → Definition → Property → Proof → Remark。
  检查与验收应验证语义归属及计数器挂载，不得仅因深度超过四层判定失败。
  有独立生命周期而非围绕该定义的性质的条目，仍按 §11.4 独立编排。
...
本章只约束 Agent 的任务协作、范围与交接，不重定义 §3–§15 的 SNL 语义、命名、
Package / Library 归属或内容标准。一般提交、验证与版本控制纪律仍按 §1–§2；
任务的授权边界优先于自行推进提交、合并或推送。

### 16.1 任务范围与并发归属

- 执行前明确本轮可写范围、目标及禁止触碰的工作区；不同 Agent 各在分配的 worktree
  和文件范围内工作，不切换、不代管或改写别人的 worktree，不把共享目录的现状推定为
  自己的授权。一个 worktree 对应一个任务、一个 Agent；分支隔离见 §2.2。
- 任务只授权所列范围内的工作。发现相关内容在另一 Agent 或另一范围内，不因其是当前
  工作的前置依赖就跨范围修改，也不得擅自将任务扩大成跨包、跨库迁移。
- 同时工作的文件或分支发生占用、存在未归属的改动时，先辨明归属并保留原状；不得以
  覆盖、回滚或替他人提交的方式「解决」冲突。未确定边界之前，暂停受影响部分并交还。

**失败模式：** 在共享 worktree 中替别的 Agent 切分支、修改或清理其改动；把任务需要
某条前置概念误当成已获授权修改其拥有包。

### 16.2 范围外前置概念的需求交接

- 若当前工作依赖尚缺的底层概念 Entry 或 Macro，先核对其语义归属与已有身份（§3、
  §4、§5、§9、§11），区分「确实缺失」与「已有但未找到」。若该概念理应归属当前
  Agent 范围外的理论、Package、Library 或其他 worker，**不得**为了让本任务通过
  而在当前库、当前宏包就地补造替身、复制定义或占用他人的归属。
- 交接时至少说明缺少什么、为什么本任务依赖它、建议的语义归属及哪些工作因此受阻；
  不确定的身份或归属明确标为待确认。只报告可核实的状态，不把未实现的 Entry、Macro、
  迁移、验证或渲染伪称为已完成；不以空壳或局部占位冒充依赖已经满足。§6.1 与 §11.3
  允许的真实目录骨架或待写子条目，也不等于一个可用的前置定义。
- 保持当前范围内可独立完成的部分与受阻部分分离；受阻部分交还协调者，由协调者另行
  安排在正确归属范围内实现、核验并对接。本章不授权当前 Agent 替协调者派发，也不
  预设另一 worker 已接受或完成需求。

**失败模式：** 下游为赶进度在自己的宏包增加上游概念的同义宏，导致一概念多身份；
把「需要上游定义」写成「上游定义已完成」，使下游验收掩盖实际缺口。

### 16.3 交还与机制待设计边界

- 交还时区分已完成且已核验的工作、未核验的工作、范围外依赖与阻塞范围，明确哪些
  事项仍须由协调者安排；不得将提出需求等同于需求已分配或已落地。若本轮被要求冻结
  交还，不再自行提交、合并、推送或接管其他范围。
- **具体交接工作流尚待设计。** 如何记录、确认、排程及追踪需求，应由协调者另行
  设计并安排；本章仅规定上述最低信息和不越权边界，**不**指定平台、状态机、工单
  格式或自动派发协议，也不声称已有此类机制。机制未定不妨碍如实报告依赖，亦不授权
  私下修改归属外资源。

**失败模式：** 用自创流程声称需求已自动分派，或以提交/推送当前工作替代协调者对
范围外前置工作作归属决定。
